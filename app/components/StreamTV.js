...
import { Search, Home, Tv, Film, Clapperboard, Heart, ListVideo, Settings, Plus, Play, Menu, X, LogIn, UserPlus, LogOut, Trash2, RefreshCw, ChevronRight } from 'lucide-react';
...
export default function StreamTV() {
  ...
  useEffect(() => {
    ...
    const stored = JSON.parse(localStorage.getItem('streamtv:favorites') || '[]');
    setFavorites(stored);
  ...
  const visible = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(x => `${x.title} ${x.group} ${x.subtitle || ''}`.toLowerCase().includes(q));
    }
    return result;
  ...
  async function authSubmit(e) {
    ...
    const fn = authMode === 'login' ? supabase.auth.signInWithPassword({ email, password }) : supabase.auth.signUp({ email, password });
  ...
  async function importPlaylist(e) {
    ...
    try {
      ...
      const imported = data.items.map(x => ({ ...x, playlist: playlistName || 'Ma playlist' }));
      setItems(prev => [...prev.filter(x => !x.imported), ...imported.map(x => ({...x, imported:true}))]);
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><button className="add-side" onClick={() => setPlaylistModal(true)}><Plus size={18}/> Ajouter une playlist</button><button onClick={() => setSection('settings')}><Settings size={18}/> Paramètres</button></div>
    </aside>
